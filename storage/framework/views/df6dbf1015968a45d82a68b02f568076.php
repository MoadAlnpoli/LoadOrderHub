...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', __('messages.title')); ?> - LoadOrderHub</title>
    
    <!-- Meta tags for SEO -->
    <?php if (! empty(trim($__env->yieldContent('meta')))): ?>
        <?php echo $__env->yieldContent('meta'); ?>
    <?php else: ?>
        <meta name="description" content="<?php echo e(__('messages.subtitle')); ?>">
        <meta name="robots" content="index, follow">
        <link rel="canonical" href="<?php echo e(url()->current()); ?>">
        <meta property="og:title" content="<?php echo $__env->yieldContent('title', __('messages.title')); ?> - LoadOrderHub">
        <meta property="og:description" content="<?php echo e(__('messages.subtitle')); ?>">
    ...
    <style>
        body {
            font-family: <?php echo e(app()->getLocale() == 'ar' ? "'Cairo', sans-serif" : "'Outfit', sans-serif"); ?>;
...
<body class="text-slate-100 min-h-screen flex flex-col antialiased bg-[#0B0F19]">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <div class="flex items-center space-x-6 rtl:space-x-reverse">
                    <a href="<?php echo e(route('home')); ?>" class="flex items-center space-x-2 rtl:space-x-reverse group shrink-0">
                    ...
                    <div class="relative group hidden md:block">
                        ...
                        <div class="absolute top-full left-0 mt-2 w-64 bg-slate-950/95 backdrop-blur-xl border border-slate-800 rounded-2xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform origin-top-left group-hover:translate-y-0 translate-y-2 z-50">
                            <div class="p-4 space-y-1">
                                <?php $navGames = \App\Models\Game::take(6)->get(); ?>
                                <?php $__currentLoopData = $navGames; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $navG): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <a href="<?php echo e(route('games.show', $navG->slug)); ?>" class="flex items-center gap-3 p-2 rounded-xl hover:bg-slate-900 transition-colors group/item">
                                    <img src="<?php echo e($navG->thumbnail_url); ?>" class="w-8 h-8 rounded-lg object-cover">
                                    <span class="text-sm font-bold text-slate-300 group-hover/item:text-violet-400 transition-colors"><?php echo e($navG->name); ?></span>
                                </a>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                <div class="border-t border-slate-800/60 mt-2 pt-2">
                                    <a href="<?php echo e(route('home')); ?>" class="block text-center text-xs font-bold text-slate-500 hover:text-violet-400 transition-colors py-2">
                                        View All Games
                                    </a>
                                </div>
                            </div>
                ...
                <div class="flex-1 max-w-sm mx-4 relative block" dir="<?php echo e(app()->getLocale() == 'ar' ? 'rtl' : 'ltr'); ?>">
                    <div class="relative">
                        <span class="absolute inset-y-0 <?php echo e(app()->getLocale() == 'ar' ? 'right-0 pr-3' : 'left-0 pl-3'); ?> flex items-center pointer-events-none text-slate-500">
                        ...
                        <input 
                            ...
                            placeholder="<?php echo e(app()->getLocale() == 'ar' ? 'البحث عن لعبة، مود...' : 'Search games, mods...'); ?>" 
                            class="w-full bg-slate-950/60 border border-slate-800 rounded-xl <?php echo e(app()->getLocale() == 'ar' ? 'pr-9 pl-4' : 'pl-9 pr-4'); ?> py-2 text-xs text-slate-200 focus:outline-none focus:border-violet-600">
                    </div>
                    <div id="global-search-results" class="absolute w-full mt-2 bg-slate-950 border border-slate-800 rounded-xl shadow-2xl overflow-hidden z-50 hidden <?php echo e(app()->getLocale() == 'ar' ? 'text-right' : 'text-left'); ?> text-xs divide-y divide-slate-800/40"></div>
                </div>
                ...
                <div class="flex items-center space-x-3 rtl:space-x-reverse relative" id="nav-actions-container">
                    <a href="<?php echo e(route('modpacks.create')); ?>" class="btn-shimmer px-4 py-2 rounded-xl bg-gradient-to-r from-violet-600 to-blue-500 hover:from-violet-500 hover:to-blue-400 text-xs text-white font-extrabold transition-all shadow-md shadow-violet-500/10 hover:shadow-violet-500/20 flex items-center gap-1.5 shrink-0">
                        <i class="fa-solid fa-plus text-[10px]"></i>
                        <span><?php echo e(app()->getLocale() == 'ar' ? 'إنشاء تجميعة' : 'Create Pack'); ?></span>
                    </a>
                    ...
                    <div id="nav-dropdown-panel" class="hidden fixed md:absolute top-0 md:top-14 right-0 w-80 md:w-72 h-full md:h-auto bg-slate-950/98 md:bg-slate-950/95 border-l md:border border-slate-800/80 shadow-2xl p-6 md:p-4 space-y-4 z-50 transition-all duration-300 transform translate-x-full md:translate-x-0 md:rounded-2xl <?php echo e(app()->getLocale() == 'ar' ? 'text-right' : 'text-left'); ?>" dir="<?php echo e(app()->getLocale() == 'ar' ? 'rtl' : 'ltr'); ?>">
                        ...
                        <?php if(auth()->guard()->guest()): ?>
                            <div class="grid grid-cols-2 gap-2 pb-3 border-b border-slate-850">
                                <a href="<?php echo e(route('login')); ?>" class="py-2 text-center rounded-xl bg-slate-900 border border-slate-805 text-slate-350 hover:text-white text-xs font-bold transition-all"><?php echo e(__('Login')); ?></a>
                                <a href="<?php echo e(route('register')); ?>" class="py-2 text-center rounded-xl bg-gradient-to-r from-violet-600 to-blue-500 text-white text-xs font-bold transition-all shadow"><?php echo e(__('Register')); ?></a>
                            </div>
                        <?php else: ?>
                            <div class="pb-3 border-b border-slate-850 flex items-center gap-3">
                                <?php if(auth()->user()->profile?->avatar): ?>
                                    <img src="<?php echo e(auth()->user()->profile->avatar); ?>" alt="" class="w-10 h-10 rounded-full object-cover border border-violet-500/30">
                                <?php else: ?>
                                ...
                                <?php endif; ?>
                                <div class="flex-1 min-w-0 <?php echo e(app()->getLocale() == 'ar' ? 'text-right' : 'text-left'); ?>">
                                    <div class="text-xs font-black text-white truncate"><?php echo e(auth()->user()->name); ?></div>
                                    <form action="<?php echo e(route('logout')); ?>" method="POST" class="mt-1">
                                        ...
                                        <button type="submit" class="text-[10px] text-slate-500 hover:text-red-400 transition-colors font-bold flex items-center gap-1">
                                            ...
                                            <span><?php echo e(app()->getLocale() == 'ar' ? 'تسجيل الخروج' : 'Logout'); ?></span>
                                        </button>
                                    </form>
                                </div>
                            </div>
                        ...
                        <div class="space-y-1 pb-3 border-b border-slate-850">
                            <div class="text-[9px] font-bold text-slate-600 uppercase tracking-widest px-2 mb-1"><?php echo e(app()->getLocale() == 'ar' ? 'الألعاب المدعومة' : 'Supported Games'); ?></div>
                            <div class="max-h-28 overflow-y-auto pr-1 space-y-1">
                                <?php $__empty_1 = true; $__currentLoopData = $navGames; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $game): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    <a href="<?php echo e(route('games.show', $game->slug)); ?>" class="flex items-center gap-2 px-2 py-1.5 rounded-lg text-slate-350 hover:text-violet-400 hover:bg-slate-900/60 text-xs transition-colors">
                                        <i class="fa-solid fa-gamepad text-[10px] text-slate-500"></i>
                                        <span><?php echo e($game->name); ?></span>
                                    </a>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                                    <div class="text-[10px] text-slate-600 px-2 py-1"><?php echo e(app()->getLocale() == 'ar' ? 'لا توجد ألعاب مضافة حالياً' : 'No games added yet'); ?></div>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <div class="space-y-1 pb-3 border-b border-slate-850 text-xs">
                            <a href="<?php echo e(route('mods.explorer')); ?>" class="flex items-center gap-2 px-2 py-2 rounded-lg text-slate-350 hover:text-violet-400 hover:bg-slate-900/60 transition-colors">
                                <i class="fa-solid fa-compass text-violet-500 w-4 text-center"></i>
                                <span><?php echo e(app()->getLocale() == 'ar' ? 'مستكشف المودات' : 'Mods Explorer'); ?></span>
                            </a>
                            <a href="<?php echo e(route('mods.trending')); ?>" class="flex items-center gap-2 px-2 py-2 rounded-lg text-slate-350 hover:text-amber-400 hover:bg-slate-900/60 transition-colors">
                                <i class="fa-solid fa-fire text-amber-500 w-4 text-center"></i>
                                <span><?php echo e(app()->getLocale() == 'ar' ? 'المودات الرائجة' : 'Trending Mods'); ?></span>
                            </a>
                            <a href="<?php echo e(route('mods.compare')); ?>" class="flex items-center gap-2 px-2 py-2 rounded-lg text-slate-350 hover:text-blue-400 hover:bg-slate-900/60 transition-colors">
                                <i class="fa-solid fa-code-compare text-blue-400 w-4 text-center"></i>
                                <span><?php echo e(app()->getLocale() == 'ar' ? 'مقارنة المودات' : 'Compare Mods'); ?></span>
                            </a>
                            <a href="<?php echo e(route('mods.top-weekly')); ?>" class="flex items-center gap-2 px-2 py-2 rounded-lg text-slate-350 hover:text-amber-300 hover:bg-slate-900/60 transition-colors">
                                <i class="fa-solid fa-trophy text-amber-400 w-4 text-center"></i>
                                <span><?php echo e(app()->getLocale() == 'ar' ? 'أفضل المودات' : 'Top Mods'); ?></span>
                            </a>

                            <?php if(auth()->guard()->check()): ?>
                                <a href="<?php echo e(route('home')); ?>" class="flex items-center gap-2 px-2 py-2 rounded-lg text-slate-350 hover:text-indigo-400 hover:bg-slate-900/60 transition-colors">
                                    <i class="fa-solid fa-house text-indigo-400 w-4 text-center"></i>
                                    <span><?php echo e(app()->getLocale() == 'ar' ? 'الرئيسية' : 'Home'); ?></span>
                                </a>
                                <a href="<?php echo e(route('profile.show', auth()->user()->id)); ?>" class="flex items-center gap-2 px-2 py-2 rounded-lg text-slate-350 hover:text-indigo-400 hover:bg-slate-900/60 transition-colors">
                                    <i class="fa-solid fa-user text-indigo-400 w-4 text-center"></i>
                                    <span><?php echo e(app()->getLocale() == 'ar' ? 'ملفي الشخصي' : 'My Profile'); ?></span>
                                </a>
                                <?php if(auth()->user()->is_admin): ?>
                                    ...
                                    ?>
                                    <a href="<?php echo e(route('admin.dashboard')); ?>" class="flex items-center justify-between gap-2 px-2 py-2 rounded-lg text-slate-350 hover:text-violet-400 hover:bg-slate-900/60 transition-colors">
                                        <div class="flex items-center gap-2">
                                            <i class="fa-solid fa-screwdriver-wrench text-violet-400 w-4 text-center"></i>
                                            <span><?php echo e(app()->getLocale() == 'ar' ? 'إدارة الموقع (Admin)' : 'Site Admin'); ?></span>
                                        </div>
                                        <?php if($adminBadgeCount > 0): ?>
                                            <span class="bg-red-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-full animate-pulse"><?php echo e($adminBadgeCount); ?></span>
                                        <?php endif; ?>
                                    </a>
                                <?php endif; ?>
                            <?php endif; ?>
                        ...
                        <div class="flex items-center justify-between text-xs pt-1">
                            <span class="text-slate-500 font-bold"><?php echo e(app()->getLocale() == 'ar' ? 'الإعدادات' : 'Settings'); ?></span>
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex gap-2">
                                    <a href="?lang=en" class="px-2.5 py-1 rounded border <?php echo e(app()->getLocale() == 'en' ? 'border-violet-500 text-violet-400 font-bold bg-violet-500/10' : 'border-slate-800 text-slate-400 hover:text-white'); ?> transition-colors">EN</a>
                                    <a href="?lang=ar" class="px-2.5 py-1 rounded border <?php echo e(app()->getLocale() == 'ar' ? 'border-violet-500 text-violet-400 font-bold bg-violet-500/10' : 'border-slate-800 text-slate-400 hover:text-white'); ?> transition-colors">AR</a>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div id="adblock-warning" class="hidden fixed inset-0 z-[100] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="glass-card max-w-md w-full rounded-2xl p-6 text-center shadow-2xl border border-violet-500/20 transform transition-all">
            ...
            <h3 class="text-xl font-bold text-white mb-2"><?php echo e(app()->getLocale() == 'ar' ? 'يرجى إيقاف مانع الإعلانات' : 'Please disable your AdBlocker'); ?></h3>
            <p class="text-sm text-slate-400 mb-6 leading-relaxed">
                <?php echo e(app()->getLocale() == 'ar' ? 'نحن نعتمد على الإعلانات لإبقاء الموقع مجانياً وتغطية تكاليف الخوادم السريعة. يرجى إضافة موقعنا للقائمة البيضاء لدعمنا!' : 'We rely on ads to keep this website free and cover our fast server costs. Please whitelist us to support our work!'); ?>
            ...
            <button onclick="document.getElementById('adblock-warning').classList.add('hidden')" class="w-full py-3 bg-gradient-to-r from-violet-600 to-blue-500 hover:from-violet-500 hover:to-blue-400 text-white rounded-xl font-bold transition-all shadow-lg shadow-violet-500/20">
                <?php echo e(app()->getLocale() == 'ar' ? 'فهمت، سأقوم بإيقافه' : 'I understand, I will disable it'); ?>
    ...
    <script>
        ...
        function reportMod(modId) {
            ...
            const url = "<?php echo e(url('/mods')); ?>/" + modId + "/report";
        ...
        function doGlobalNavSearch(query) {
            ...
            searchTimeout = setTimeout(() => {
                fetch(`/api/search?q=${encodeURIComponent(query)}`)
                    ...
                    .then(data => {
                        if (data.length === 0) {
                            dropdown.innerHTML = `<div class="px-4 py-3 text-slate-500 text-center"><?php echo e(app()->getLocale() == 'ar' ? 'لا توجد نتائج' : 'No results found'); ?></div>`;
                        } else {
        ...
        <?php if(session('success')): ?>
            document.addEventListener('DOMContentLoaded', () => showToast("<?php echo e(session('success')); ?>", 'success'));
        <?php endif; ?>
        <?php if(session('error')): ?>
            document.addEventListener('DOMContentLoaded', () => showToast("<?php echo e(session('error')); ?>", 'error'));
        <?php endif; ?>

        // Global Ad Click Tracking
        function trackAdClick(adName) {
            fetch('<?php echo e(route('ads.track')); ?>', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
            ...
            if (!localStorage.getItem('cookieConsent')) {
                ...
                banner.innerHTML = `
                    <div class="text-sm text-slate-300">
                        ...
                        We use cookies and third-party advertising to improve your experience. By continuing to use our site, you accept our <a href="<?php echo e(route('privacy')); ?>" class="text-violet-400 hover:underline">Privacy Policy</a>.
                    </div>
...
<?php /**PATH C:\lravle\taskmn\resources\views/layouts/app.blade.php ENDPATH**/ ?>