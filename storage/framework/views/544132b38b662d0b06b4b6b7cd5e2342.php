...
<div class="max-w-6xl mx-auto space-y-6">
    <div class="flex items-center justify-between border-b border-slate-800 pb-4">
        ...
        <a href="<?php echo e(route('admin.dashboard')); ?>" class="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-xs font-bold transition-all">العودة للوحة التحكم</a>
    </div>
    ...
    <div class="glass-card p-6 rounded-3xl border border-slate-850">
        ...
        <form action="<?php echo e(route('admin.ads.store')); ?>" method="POST" class="space-y-4">
            <?php echo csrf_field(); ?>
                ...
                <form action="<?php echo e(route('admin.ads.update', $ad->id)); ?>" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="name" value="<?php echo e($ad->name); ?>" class="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-2 text-white focus:outline-none focus:border-blue-500" required>
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-slate-400">الحالة</label>
                            <div class="pt-2">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="is_active" value="1" <?php echo e($ad->is_active ? 'checked' : ''); ?> class="w-4 h-4 rounded border-slate-800 bg-slate-900 text-blue-500 focus:ring-blue-500">
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <textarea name="code" rows="3" class="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-2 text-white focus:outline-none focus:border-blue-500" dir="ltr" required><?php echo e($ad->code); ?></textarea>
                        </div>
                    </div>
                    <div class="flex justify-end gap-2 mt-4 pt-4 border-t border-slate-800">
                        ...
                        <button type="button" onclick="if(confirm('هل أنت متأكد من الحذف؟')) document.getElementById('delete-ad-<?php echo e($ad->id); ?>').submit();" class="px-5 py-2 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-xl text-xs font-bold transition-all border border-red-500/20">حذف</button>
                    </div>
                </form>
                <form id="delete-ad-<?php echo e($ad->id); ?>" action="<?php echo e(route('admin.ads.delete', $ad->id)); ?>" method="POST" class="hidden">
                    <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\lravle\taskmn\resources\views/admin/ads.blade.php ENDPATH**/ ?>