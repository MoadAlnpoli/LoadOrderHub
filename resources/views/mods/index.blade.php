...
@section('title', __('messages.game_mods', ['game' => $game->name] ?? ['game' => $game->name]))
...
<div class="space-y-8">
    ...
    <nav class="flex text-xs text-slate-500 space-x-2 rtl:space-x-reverse">
        ...
        <span>/</span>
        <a href="{{ route('games.show', $game->slug) }}" class="hover:text-slate-300">{{ $game->name }}</a>
        <span>/</span>
    ...
    <div class="relative rounded-3xl overflow-hidden bg-slate-900 border border-slate-800 p-6 md:p-8 flex flex-col md:flex-row items-center md:items-start gap-6">
        ...
        <div class="space-y-3 text-center md:text-left rtl:md:text-right flex-grow">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <h1 class="text-2xl md:text-3xl font-extrabold text-white">{{ $game->name }} - Mods Library</h1>
                <a href="{{ route('games.show', $game->slug) }}" class="inline-flex items-center justify-center px-4 py-2 rounded-xl bg-slate-800 border border-slate-700 hover:border-violet-600/40 text-slate-300 hover:text-white font-semibold text-xs tracking-wide transition-all shadow-md">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="glass-card p-6 rounded-2xl border border-slate-800 space-y-5">
                ...
                <div class="space-y-1">
                    ...
                    <div class="relative">
                        <input type="text" id="search-input" value="{{ $search }}" placeholder="e.g. Harmony" class="w-full bg-slate-950 border border-slate-800 rounded-xl pl-9 pr-4 py-2.5 text-xs text-slate-200 focus:outline-none focus:border-violet-600">
        ...
        <div class="lg:col-span-3 space-y-6">
            <div id="mods-list-container" class="transition-opacity duration-200">
                @include('mods.partials.mods_list_table', ['mods' => $mods, 'game' => $game])
            </div>
        </div>
